...
//
// genai-model-eval-list-runs is the tool under test because it is the list
// projection this package hand-writes ({runs, count}) over a godo resource
// that carries a Timestamp, which is the field kind most likely to reflect to
// a schema the payload does not satisfy.
func TestStructuredOutputSatisfiesDeclaredSchema(t *testing.T) {
	...
	raw, err := json.Marshal(svr.HandleMessage(ctx,
		[]byte(`{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"genai-model-eval-list-runs","arguments":{}}}`)))
	...
	var resp struct {
		...
		Result struct {
			...
			StructuredContent modelEvalRunList  `json:"structuredContent"`
		} `json:"result"`
	}
	...
	require.Len(t, resp.Result.StructuredContent.Runs, 1)
	require.Equal(t, "run-1", resp.Result.StructuredContent.Runs[0].EvalRunUuid)

	// The text block is retained for backwards compatibility and carries the
	// same payload.
	require.Len(t, resp.Result.Content, 1)
	var fromText modelEvalRunList
