...
//
// Note: this package also exposes a Prompt (model-comparison) via Prompts();
// prompts are not tools and are intentionally not covered by this contract.
var expectedAnnotations = map[string]struct {
	...
	// model_tools.go
	"inference-model-catalog-search":   {true, false, true, false, common.OpRead, common.RiskLow, false, false},
	"inference-model-catalog-get-card": {true, false, true, false, common.OpRead, common.RiskLow, false, false},
}
...
func TestToolAnnotations(t *testing.T) {
	...
	all = append(all, NewModelTool(clientFn).Tools()...)
