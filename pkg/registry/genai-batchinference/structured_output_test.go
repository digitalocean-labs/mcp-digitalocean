...
func TestStructuredOutputSatisfiesDeclaredSchema(t *testing.T) {
	...
	mockBatch := NewMockBatchInferenceService(ctrl)
	mockBatch.EXPECT().
		GetJob(gomock.Any(), testBatchQueued.BatchID).
		Return(testBatchQueued, nil, nil).
		Times(1)

	svr := server.NewMCPServer("test", "test", server.WithOutputSchemaValidation())
	svr.AddTools(setupToolWithMock(mockBatch).Tools()...)
	...
	raw, err := json.Marshal(svr.HandleMessage(ctx,
		[]byte(`{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"genai-batch-inference-get",`+
			`"arguments":{"BatchID":"`+testBatchQueued.BatchID+`"}}}`)))
	...
	var resp struct {
		...
		Result struct {
			...
			StructuredContent godo.Batch        `json:"structuredContent"`
		} `json:"result"`
	}
	...
	require.Equal(t, testBatchQueued.BatchID, resp.Result.StructuredContent.BatchID)
	require.Equal(t, "queued", resp.Result.StructuredContent.Status)

	// The text block is retained for backwards compatibility and carries the
	// same payload.
	require.Len(t, resp.Result.Content, 1)
	var fromText godo.Batch
