...
func TestStructuredOutputSatisfiesDeclaredSchema(t *testing.T) {
	...
	mockOneClick.EXPECT().
		...
		Return([]*godo.OneClick{{Slug: "wordpress", Type: "droplet"}}, nil, nil).
		Times(1)

	svr := server.NewMCPServer("test", "test", server.WithOutputSchemaValidation())
	svr.AddTools(setupOneClickToolWithMock(mockOneClick).Tools()...)
	...
	raw, err := json.Marshal(svr.HandleMessage(ctx,
		[]byte(`{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"marketplace-1-click-list","arguments":{}}}`)))
	...
	var resp struct {
		...
		Result struct {
			...
			StructuredContent oneClickList      `json:"structuredContent"`
		} `json:"result"`
	}
