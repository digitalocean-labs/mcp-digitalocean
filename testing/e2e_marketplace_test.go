...
// TestInstallKubernetesApps tests installing 1-click apps on a Kubernetes cluster
func TestInstallKubernetesApps(t *testing.T) {
	...
	// Get available Kubernetes 1-click apps to find a valid app slug
...
// TestInstallMultipleKubernetesApps tests installing multiple 1-click apps at once
func TestInstallMultipleKubernetesApps(t *testing.T) {
	ctx := context.Background()
	c := initializeClient(ctx, t)
	defer c.Close()
	...
	// Create a test Kubernetes cluster
	shortID := fmt.Sprintf("%08x", uuid.New().ID())
	clusterName := fmt.Sprintf("mcp-marketplace-multi-%s", shortID)
	...
	// Get available Kubernetes 1-click apps
	listResp, err := c.CallTool(ctx, mcp.CallToolRequest{
		Params: mcp.CallToolParams{
			Name: "1-click-list",
			Arguments: map[string]interface{}{
	...
	// Get up to 2 app slugs for testing multiple installs
